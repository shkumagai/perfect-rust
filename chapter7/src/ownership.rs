...
pub fn ownership_3() {
    ...
    let x = String::from("ABC");    // 変数xがインスタンスの所有権を持っている
    println!("x = {:?}", x);        // 変数xの値を出力
    let y = x.clone();  // clone()メソッドで変数xの複製を作成して代入する
    println!("x = {:?}", x);        // 変数xの値を出力
    println!("y = {:?}", y);        // 変数yの値を出力
}
...
/// ## 7-4.所有権
/// ### リスト7.1９ 引数渡しによる所有権の移動
#[allow(dead_code)]
fn print_message(message: &String) {
    println!("message = {}", message);
}
/// ## 7-4.所有権
/// ### リスト7.１９ 引数渡しによる所有権の移動
#[allow(dead_code)]
pub fn ownership_5() {
    // 文字列を生成して変数xへ代入
    let x = String::from("ABC");    // 変数xがインスタンスの所有権を持っている
    print_message(&x);   // print_message()関数に変数xの参照を渡す
    println!("x = {:?}", x);
}

/// ## 7-4.所有権
/// ### リスト7.２０ リターンによる所有権の移動
#[allow(dead_code)]
fn message() -> String {
