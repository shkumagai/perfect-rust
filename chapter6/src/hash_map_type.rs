...
pub fn get_and_change() {
    ...
    for (k, v) in p_iter {  // キーと値を出力する
        println!("key:{}, value:{}", &k, &v);
    }
    // キーがi32型、値が&str型のHashMapを生成する
    let mut map_x: HashMap<i32, &str> = HashMap::new();
    map_x.extend([(1, "ABC"), (2, "DEF"), (10, "XYZ")]);
    if let Some(v) = map_x.get_mut(&2) {    // 指定されたキーで値を取得して変更する
        *v = "あいうえお";
    }
    println!("{:?}", &map_x);
    // キーがi32型、値がi32型のHashMapを生成する
    let mut map_y: HashMap<i32, i32> = vec![(1, 10), (2, 11), (3, 12), (4, 13)].into_iter().collect();
    let p_iter = map_y.iter_mut();  // イテレータを取得する
    for (_, v) in p_iter {  // 値が偶数なら10倍する
        if *v % 2 == 0 {
            *v *= 10;
        }
    }
...
pub fn remove() {
    ...
    let mut map_x: HashMap::<i32, &str> = HashMap::new();
    map_x.extend([(1, "ABC"), (2, "DEF"), (10, "XYZ")]);
    let v: String = match map_x.remove(&2) {    // 指定されたキーのペアを削除する
        None => "指定されたキーのペアが見つかりません。".to_string(),
        Some(x) => format!("値:{}が削除されました。", &x)
    };
    println!("{}", &v);
    println!("{:?}", &map_x);
    map_x.clear();  // 要素をクリアする
    println!("clear() = {:?}", &map_x);
}
