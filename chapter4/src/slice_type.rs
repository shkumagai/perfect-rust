...
pub fn multibyte_slice() {
    ...
    let slice = &company_name[..12];    // 株式会社の範囲をスライスで取得する
    println!("参照範囲 = {:?}, 大きさ = {}", slice, slice.len());
    let slice = &company_name[12..];    // フルネスの範囲をスライスで取得する
    println!("参照範囲 = {:?}, 大きさ = {}", slice, slice.len());
}

/// ### 4-7.スライス型
/// #### リスト４．２５ ファットポインタを確認する
#[allow(dead_code)]
pub fn fat_pointer() {
...
pub fn methods_3() {
    ...
    let slice = &vec[..];
    let chks = slice.chunks(3); // 指定したサイズに分割したスライスを返す
    for chk in chks {
        println!("chunks()  = {:?}", chk);
    }
    ...
    let array = [100, 101, 102, 103, 104];
    let slice: &[i32] = &array[..];
    // 4の倍数要素だけを除外したサブスライスのイテレータを返す
    let seps = slice.split(|value|{
        value % 4 == 0          // 要素は4の倍数か？
    });
    for spt in seps {
        println!("split()   = {:?}", spt);
