...
pub fn methods_2() {
    ...
    let result = str_value.find("田");          // 指定された文字列の位置を示す
    println!("find()        = {:?}", result);

    let str_value = "sample.txt";
    let result = str_value.ends_with(".txt");   // 終わりが指定された文字列と一致するか
    println!("ends_with()   = {:?}", result);

    let result = str_value.starts_with("sample");   // 始まりが指定された文字列と一致するか
    println!("start_with()  = {:?}", result);

    let result = str_value.is_empty();          // 文字列が空であるか
...
pub fn methods_3() {
    ...
    let result = str_value.get(0..=3).unwrap(); // 0〜4番目の要素を取得する
